...
const ReportOrder = () => {
  ...
  const stats = [
    { label: 'Total Orders', value: '142', change: '+15%', color: 'bg-gradient-to-r from-[#D1FAE5] to-[#A7F3D0]' },
    { label: 'Completed', value: '128', change: '90%', color: 'bg-gradient-to-r from-[#6EE7B7] to-[#34D399]' },
    { label: 'Total Value', value: '₹9.8 L', change: 'This Month', color: 'bg-gradient-to-r from-[#10B981] to-[#059669]' },
    { label: 'Pending', value: '14', change: 'In Progress', color: 'bg-gradient-to-r from-[#047857] to-[#065F46]' }
  ];

  const orders = [
    { id: 'ORD-001', client: 'ABC Exports', date: '2024-01-15', items: '12', value: '₹45,000', status: 'Completed' },
    { id: 'ORD-002', client: 'XYZ Trading', date: '2024-01-16', items: '8', value: '₹32,000', status: 'Pending' },
    { id: 'ORD-003', client: 'Global Foods', date: '2024-01-17', items: '15', value: '₹58,300', status: 'Completed' }
  ];
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="bg-white rounded-2xl overflow-hidden border border-[#D0E0DB]">
            ...
            <tbody>
              {orders.map((order, index) => (
                <tr key={index} className={`border-b border-[#D0E0DB] hover:bg-[#F0F4F3] transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-[#F0F4F3]/30'}`}>
                  <td className="px-6 py-4 text-sm text-[#0D5C4D]">{order.id}</td>
                  <td className="px-6 py-4 font-semibold text-[#0D5C4D]">{order.client}</td>
                  <td className="px-6 py-4 text-sm text-[#0D5C4D]">{order.date}</td>
                  <td className="px-6 py-4 text-sm text-[#0D5C4D]">{order.items}</td>
                  <td className="px-6 py-4 text-sm font-semibold text-[#0D5C4D]">{order.value}</td>
                  <td className="px-6 py-4">
                    <span className={`px-3 py-1.5 rounded-full text-xs font-medium flex items-center gap-1 w-fit ${order.status === 'Completed' ? 'bg-[#4ED39A] text-white' : 'bg-red-500 text-white'}`}>
        ...
        <div className="flex items-center justify-between px-6 py-4 bg-[#F0F4F3] border-t border-[#D0E0DB]">
          <div className="text-sm text-[#6B8782]">Showing 3 of 142 orders</div>
