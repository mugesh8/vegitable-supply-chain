...
const ThirdPartyDetails = () => {
  ...
  useEffect(() => {
    const fetchthirdParty = () => {
      const thirdPartyInfo = thirdPartyData[id];
      if (thirdPartyInfo) {
        setthirdParty(thirdPartyInfo);
      }
    };
    ...
    return (
      <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading thirdParty details...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4 md:p-6 lg:p-8">
      ...
      <div className="flex flex-wrap gap-3 mb-6">
        ...
        <button 
          onClick={() => navigate(`/thirdPartys/${id}/orders`)}
        ...
        <button 
          onClick={() => navigate(`/thirdPartys/${id}/payout`)}
          className="px-6 py-2.5 bg-white text-gray-700 border border-gray-300 rounded-lg font-medium hover:bg-gray-50 transition-colors"
        >
          Payout
        </button>
      ...
      <div className="rounded-lg shadow-sm p-6 mb-6">
        <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
          <div className="w-24 h-24 bg-teal-800 rounded-full flex items-center justify-center flex-shrink-0">
            <span className="text-white text-3xl font-bold">{thirdParty?.name?.substring(0, 2).toUpperCase() || 'SP'}</span>
          </div>
          
          <div className="flex-1">
            <h2 className="text-2xl font-bold text-gray-800 mb-2">{thirdParty?.name || 'N/A'}</h2>
            <p className="text-gray-600 mb-2">thirdParty ID: {thirdParty?.id || 'N/A'}</p>
          ...
          <div className="flex flex-col gap-2">
            ...
            <span className={`px-4 py-1 rounded-full text-sm font-medium border flex items-center gap-2 ${
              ...
              <span className={`w-2 h-2 rounded-full ${
                thirdParty?.status === 'Active' ? 'bg-green-500' : 'bg-red-500'
              }`}></span>
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {thirdParty?.products ? thirdParty.products.split(', ').map((item, index) => (
            ...
            <div>
              ...
              <p className="text-gray-800">{thirdParty?.supplyFrequency || 'N/A'}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-teal-50 rounded-lg shadow-sm p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-gray-800">{thirdParty?.region || 'N/A'}</p>
            </div>
            
            <div>
              <p className="text-xs font-medium text-gray-500 uppercase mb-1">Full Address</p>
              <p className="text-gray-800">{thirdParty?.address || 'N/A'}</p>
            </div>
          </div>
        </div>
      </div>
